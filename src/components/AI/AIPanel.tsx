/**
 * AI Panel
 * 
 * Main AI interface panel that combines input, loading indicator, and history.
 */

import React from 'react';
import { AICommandInput } from './AICommandInput';
import { AILoadingIndicator } from './AILoadingIndicator';
import { AICommandHistory } from './AICommandHistory';

interface AIPanelProps {
  className?: string;
}

export function AIPanel({ className = '' }: AIPanelProps) {
  return (
    <div className={`flex flex-col gap-3 ${className}`}>
      {/* Title */}
      <div className="flex items-center gap-2">
        <svg 
          className="w-6 h-6 text-blue-600" 
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
        >
          <path 
            strokeLinecap="round" 
            strokeLinejoin="round" 
            strokeWidth={2} 
            d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" 
          />
        </svg>
        <h3 className="text-lg font-semibold text-gray-900">
          AI Assistant
        </h3>
      </div>

      {/* Description */}
      <p className="text-sm text-gray-600">
        Ask the AI to create or modify shapes on your canvas using natural language.
      </p>

      {/* Examples */}
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
        <div className="text-xs font-medium text-gray-700 mb-2">
          Example commands:
        </div>
        <div className="space-y-1 text-xs text-gray-600">
          <div>• "Create a blue rectangle at 200, 300"</div>
          <div>• "Add a red circle in the center"</div>
          <div>• "Create a login form"</div>
          <div>• "Create a 3x3 grid of squares"</div>
          <div>• "Move the blue shape to 500, 400"</div>
        </div>
      </div>

      {/* Loading Indicator */}
      <AILoadingIndicator />

      {/* Command Input */}
      <AICommandInput />

      {/* Command History */}
      <AICommandHistory className="mt-2" />

      {/* Tips */}
      <div className="text-xs text-gray-500 mt-2">
        <strong>Tips:</strong> Be specific with colors, positions, and sizes. 
        The AI works best with clear, detailed instructions.
      </div>
    </div>
  );
}

